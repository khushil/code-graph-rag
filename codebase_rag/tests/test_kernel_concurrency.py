...
def test_spinlock_detection(parser, queries):
    ...
    assert any(
        r[0] == "critical_function" and r[1] == "LOCKS" and r[3] == "my_lock"
        for r in lock_rels
    )
    assert any(
        r[0] == "critical_function" and r[1] == "UNLOCKS" and r[3] == "my_lock"
        for r in lock_rels
    )


def test_mutex_detection(parser, queries):
    ...
    mutex_nodes = [
        n
        for n in nodes
        if n.node_type == "concurrency_primitive"
    ...
    my_mutex = next(
        (
            n
            for n in nodes
            if n.node_type == "concurrency_primitive" and n.name == "my_mutex"
        ),
        None,
    )
    ...
    lock_rels = [r for r in relationships if r[1] in ["LOCKS", "UNLOCKS"]]
    assert any(
        r[0] == "protected_operation" and r[1] == "LOCKS" and r[3] == "my_mutex"
        for r in lock_rels
    )


def test_rwlock_detection(parser, queries):
    ...
    data_lock = next(
        (
            n
            for n in nodes
            if n.node_type == "concurrency_primitive" and n.name == "data_lock"
        ),
        None,
    )
...
def test_critical_section_tracking(parser, queries):
    ...
    lock_ops = [
        (r[0], r[1], r[3]) for r in relationships if r[1] in ["LOCKS", "UNLOCKS"]
    ]
...
def test_nested_locks(parser, queries):
...
def test_trylock_operations(parser, queries):
